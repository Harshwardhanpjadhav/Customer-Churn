    ...
    def initiate_data_transformation(self) -> DataTransformationArtifact:
        try:
            ...
            input_feature_train_df = train_df.drop(
                columns=TAREGT_COLUMN_NAME, axis=1)
            target_feature_train_df = train_df[TAREGT_COLUMN_NAME]
            target_feature_train_df = label_encoder_obj.fit_transform(
                target_feature_train_df)

            logging.info(f"Input Datatype {type(input_feature_train_df)}")
            logging.info(
                f"Output train Datatype {type(target_feature_train_df)}")
            ...
            target_feature_test_df = label_encoder_obj.fit_transform(
                target_feature_test_df)

            logging.info(f"Input test Datatype {type(input_feature_test_df)}")
            logging.info(
                f"Output test Datatype {type(target_feature_test_df)}")
            ...
            logging.info(
                "Applying preprocessing object on Input and Testing datasets >>>")
            ...
            logging.info(
                f"Input train Datatype {type(input_feature_train_arr)}")
            ...
            target_feature_train_array = labelencoder.fit_transform(
                target_feature_train_df)
            target_feature_test_array = labelencoder.transform(
                target_feature_test_df)
            ...
            save_object(
                self.data_transformation_config.data_preporcessing_object_file_path, preprocessing_obj,)
            save_object(
                self.data_transformation_config.data_preporcessing_object, preprocessing_obj)
            save_object(
                self.data_transformation_config.data_labelencoder_object_file_path, label_encoder_obj,)
            logging.info("Completed saving numpy data And labelEncoder")
